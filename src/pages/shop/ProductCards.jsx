...
const ProductCards = ({ filteredProducts }) => {
    ...
    const handleAddToCart = (product) => {
        // 1. Check koro product-e size array ache kina
        // Jodi thake, tobe prothom element-ta (index 0) nibe, na hole "N/A"
        const selectedSize = product.size && Array.isArray(product.size)
            ? product.size[0]
            : product.size;

        // 2. Notun ekti object banao jekhane 'size' hobe sudhu ekti string
        const productWithSingleSize = {
            ...product,
            size: selectedSize // Puro array replace hoye ekti single size hobe
        };

        // 3. Ebar dispatch koro
            ...
            return (
                <div key={index} className="group pb-10">
                    <div className="relative overflow-hidden bg-gray-100 aspect-[3/4]">
                        ...
                        {/* Right Side: Dynamic Percentage Label */}
                        {discount && (
